...
module.exports = {
  async listTournaments({ query }, res) {
    // Descrição:
    ...
    try {
      ...
      if (category) {
        where.push("CATEGORIA.NOME = :category")
        bind.push(category)
      }
      if (modality) {
  ...
  async createTournament({ body }, res) {
    ...
    try {
      ...
      if (!name || !description || award === undefined || !rules || !category || !date)
      ...
      `, [rules, Number(award), name, description, date, categoryCode], { autoCommit: true })
    ...
    try {
      ...
      if (body.player) {
        ...
        if (playerCategoryCode !== tournamentCategoryCode)
          return res.status(400).json({ "erro": "Jogador deve permanecer à categoria do campeonato" })
        await connection.execute(`
          INSERT INTO inscricao_jogador VALUES (
            null, to_date(:subscriptionDate, 'yyyy-mm-dd'), :playerCode, :tournamentCode
          )
        `, [date, body.player, tournament], { autoCommit: true })
      } else if (body.team) {
        const result = await connection.execute(`
        ...
        if (teamCategoryCode !== tournamentCategoryCode)
          return res.status(400).json({ "erro": "Equipe deve permanecer à categoria do campeonato" })
        await connection.execute(`
          INSERT INTO inscricao_equipe VALUES (
            null, to_date(:subscriptionDate, 'yyyy-mm-dd'), :teamCode, :tournamentCode
          )
        `, [date, body.team, tournament], { autoCommit: true })
      } else
        return res.status(400).json({ "erro": "Entradas inválidas" })
